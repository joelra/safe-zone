...
public class GameplayConfig {
	...
	public static final int DEFAULT_CLAIM_EXPIRY_DAYS = 30;
	...
	public void ensureDefaults() {
		if (!isValidItemId(this.claimWandItemId)) {
	...
	public Item claimWandItem() {
		return BuiltInRegistries.ITEM.stream()
			.filter(item -> BuiltInRegistries.ITEM.getKey(item).toString().equals(this.claimWandItemId))
			.findFirst()
			.orElse(Items.GOLDEN_HOE);
	}

	public ItemStack createClaimWandStack() {
		return new ItemStack(claimWandItem());
	}

	public String claimWandName() {
		return createClaimWandStack().getHoverName().getString();
	}
